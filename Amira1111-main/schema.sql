...
CREATE TABLE public.items (
  ...
  -- Pieces per carton
  bag_size INTEGER DEFAULT 0 -- Pieces per bag
);
-- 3. Inventory (Current Stock)
CREATE TABLE public.inventory (
  warehouse_id INTEGER فهمت قصدك تماماً ، أنت تريد تحويل النظام من نظام "العدد البسيط" إلى نظام "معامل التحويل" (Unit Conversion).بمعنى أن المخزن دائماً "يفكر" بـ القطعة (الوحدة الصغرى) ، لكنه يتيح لك التعامل بالكرتونة أو الكيس ويفهم تلقائياً كم قطعة بداخلهم.إليك الخطوات البرمجية والمنطقية لتنفيذ هذا في قاعدة بياناتك (Supabase) ونظامك: 1.تعديل جدول الأصناف (Items Table) يجب أن يعرف كل صنف "كم قطعة في الكرتونة" و "كم قطعة في الكيس".سنضيف أعمدة جديدة لجدول items: code SQL
  ALTER TABLE public.items
  ADD COLUMN pieces_per_carton INTEGER DEFAULT 1,
    -- كم قطعة داخل الكرتونة
  ADD COLUMN pieces_per_bag INTEGER DEFAULT 1;
-- كم قطعة داخل الكيس
2.المنطق الحسابي (كيف يفكر النظام ؟) عندما تقوم بعملية "صرف" أو "إضافة" ، المعادلة في البرمجة (Frontend / Backend) ستكون كالتالي: إذا اخترت قطعة: الكمية المخصومة = العدد الذي كتبته.إذا اخترت كرتونة: الكمية المخصومة = العدد الذي كتبته × pieces_per_carton.إذا اخترت كيس: الكمية المخصومة = العدد الذي كتبته × pieces_per_bag.3.مثال عملي بالكود (SQL) لتعديل البيانات التي أضفناها: لنفترض أن "الجوانتيات" (Gloves) الكرتونة فيها 10 علب ، والعلبة فيها 50 قطعة.ستقوم بتحديث الصنف هكذا: code SQL
UPDATE public.items
SET pieces_per_carton = 50
WHERE name = 'GLOVES L';
4.كيف تظهر في صفحة "إضافة / تحويل مخزون" ؟ (شرح الصورة التي أرسلتها) في الصورة التي أرفقتها ، عند اختيار الصنف: في البرمجة (Frontend): يجب أن يسحب النظام بيانات الصنف المختار (بما فيها pieces_per_carton).عند اختيار الوحدة (قطعة / كرتونة): إذا اختار المستخدم "كرتونة" وكتب "1".النظام قبل أن يرسل الطلب لـ Supabase يضرب 1 * 50.يتم إرسال رقم 50 لخانة الـ quantity في جدول الـ inventory.نصيحتي لك لتطوير الموقع: أنت تحتاج لإضافة خانة في "إدارة الأصناف" تسمى "تعبئة الوحدة".عند إضافة صنف جديد ، يظهر سؤال: "الكرتونة تحتوي على كم قطعة؟".هذا الرقم هو الذي سيستخدمه النظام كـ "مُعامل ضرب" (Multiplier) عند الصرف أو التحويل.هل تريد مني كتابة كود (Trigger) لـ Supabase يقوم بهذه الحسبة تلقائياً فور الإضافة ؟ (
  بمعنى أنك ترسل له "الوحدة" و "الكمية" وهو يحسب الصافي ويخصمه من الرصيد
...
VALUES ('المخزن الرئيسي', 'main'),
  ...
  ('عيادة 5', 'clinic');
-- 2. Items
INSERT INTO public.items (id, item_name, item_group, unit_type, sale_price)
VALUES (262, 'ENDO FILL', 'مجموعة االسنان', 'PK', 195.00),
  (
    264,
    'TETRIC N- CERAM REFILL A1',
    'مجموعة االسنان',
    'PK',
    85.00
  ),
...
VALUES (p_to_warehouse_id, p_item_id, p_qty) ON CONFLICT (warehouse_id, item_id) DO
UPDATE
SET quantity = public.inventory.quantity + p_qty;
INSERT INTO public.transactions (
    type,
    from_warehouse_id,
    to_warehouse_id,
...
INSERT INTO public.inventory (warehouse_id, item_id, quantity)
VALUES (p_warehouse_id, p_item_id, p_qty) ON CONFLICT (warehouse_id, item_id) DO
UPDATE
SET quantity = public.inventory.quantity + p_qty;
INSERT INTO public.transactions (
    type,
...
SELECT w.id,
  i.id,
  100 -- Starting with 100 pieces for everything
FROM public.warehouses w,
  public.items i ON CONFLICT (warehouse_id, item_id) DO
UPDATE
SET quantity = EXCLUDED.quantity;
-- Analytics Views (Re-verified)
CREATE OR REPLACE VIEW public.monthly_clinic_report AS
SELECT w.name AS clinic_name,
